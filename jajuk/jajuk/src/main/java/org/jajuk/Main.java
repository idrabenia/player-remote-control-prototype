...
public final class Main {
  ...
  private static Chat remoteChat;
  ...
  private static void initJabber() throws Exception {
      ...
      remoteChat = conn2.getChatManager().createChat("drabenia-test2@jabber.ccc.de", new MessageListener() {
          @Override
          public void processMessage(Chat chat, Message message) {
              System.out.println(message.getBody());
              if ("Play!".equals(message.getBody())) {
                  QueueModel.playNext();
              }

              if ("Stop!".equals(message.getBody())) {
